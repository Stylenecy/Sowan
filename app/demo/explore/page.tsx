...
import { Search, MapPin, Star, Calendar, CheckCircle, ArrowRight, X, Clock, Video, MessageSquare } from "lucide-react";
...
const MENTORS = [
    {
        ...
        online: true,
        searchTerms: ["opa adriel", "adriel", "sejarah jawa", "bahasa jawa", "yogyakarta", "pakar sejarah", "mentor jawa"].join(" "),
    },
    {
        id: 2,
        name: "Kak Maya",
        ...
        online: false,
        searchTerms: ["kak maya", "maya", "bahasa jawa", "sastra jawa", "jakarta", "ahli bahasa"].join(" "),
    },
    {
        id: 3,
        name: "Pak Budi",
        ...
        online: true,
        searchTerms: ["pak budi", "budi", "kearifan lokal", "budaya jawa", "bandung", "pakar budaya"].join(" "),
    },
];

const STEPS_ID = [
    { icon: "🔍", title: "Cari Mentor", desc: "Ketik nama atau topik yang Anda minati di kolom pencarian.", subStep: 'search' },
    { icon: "🏙️", title: "Filter Kota", desc: "Pilih kota asal mentor yang Anda inginkan.", subStep: 'filter' },
    { icon: "👤", title: "Pilih Mentor", desc: "Klik kartu mentor untuk melihat profil lengkap.", subStep: 'card' },
    { icon: "📅", title: "Buat Jadwal", desc: "Pilih waktu dan konfirmasi booking di panel samping.", subStep: 'book' },
    { icon: "🎥", title: "Masuk Ruang", desc: "Gunakan kode booking untuk masuk ke ruang video.", subStep: 'room' },
];

const STEPS_EN = [
    { icon: "🔍", title: "Search Mentor", desc: "Type the name or topic you're interested in.", subStep: 'search' },
    { icon: "🏙️", title: "Filter City", desc: "Select the mentor's city.", subStep: 'filter' },
    { icon: "👤", title: "Choose Mentor", desc: "Click mentor card to see full profile.", subStep: 'card' },
    { icon: "📅", title: "Book Schedule", desc: "Select time and confirm booking in the side panel.", subStep: 'book' },
    { icon: "🎥", title: "Join Room", desc: "Use booking code to enter the video room.", subStep: 'room' },
];

const TIME_SLOTS_ID = ["09:00", "10:00", "11:00", "14:00", "15:00", "16:00"];
const TIME_SLOTS_EN = ["09:00", "10:00", "11:00", "14:00", "15:00", "16:00"];

type SubStep = 'search' | 'filter' | 'card' | 'book' | 'room' | 'done';
...
export default function DemoExplorePage() {
    ...
    const [activeFilter, setActiveFilter] = useState<'all' | 'Yogyakarta' | 'Jakarta' | 'Bandung'>('all');
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedMentor, setSelectedMentor] = useState<typeof MENTORS[0] | null>(null);
    const [bookingStep, setBookingStep] = useState<'time' | 'confirm' | 'success'>('time');
    ...
    const TIME_SLOTS = isID ? TIME_SLOTS_ID : TIME_SLOTS_EN;
    ...
    const filteredMentors = useMemo(() => {
        return MENTORS.filter(m => {
            ...
            const matchesSearch = searchQuery === '' || m.searchTerms.toLowerCase().includes(searchQuery.toLowerCase());
            return matchesCity && matchesSearch;
        });
    }, [activeFilter, searchQuery]);

    const handleFilterClick = (city: typeof activeFilter) => {
        setActiveFilter(city);
        if (currentStep === 1) {
            const timer = setTimeout(() => {
                if (city === 'Yogyakarta' && filteredMentors.some(m => m.city === 'Yogyakarta')) {
                    setCurrentStep(2);
                }
            }, 500);
    ...
    const getSubStep = (): SubStep => {
        if (showSidePanel) return bookingStep === 'success' ? 'room' : 'book';
        if (showRoomEntry) return 'room';
        if (showFeedback) return 'done';
        return stepData.subStep as SubStep;
    };

    const bubbleText = () => {
        if (showSidePanel && bookingStep === 'time') return isID ? "Pilih waktu yang tersedia" : "Select available time";
        if (showSidePanel && bookingStep === 'confirm') return isID ? "Konfirmasi booking Anda" : "Confirm your booking";
        if (showSidePanel && bookingStep === 'success') return isID ? "✓ Booking berhasil! Masuk ke ruang video" : "✓ Booking confirmed! Enter video room";
        ...
        <main className="min-h-screen w-full bg-[#FAF9F6] pt-[72px]">
            ...
            <div className="fixed left-1/2 -translate-x-1/2 z-50 transition-all duration-500"
                ...
                <div className="bg-accent text-white px-8 py-5 rounded-3xl shadow-2xl flex items-center gap-4"
                    ...
                    <div className="min-w-0">
                        <p className="font-black text-base whitespace-nowrap">{bubbleText()}</p>
                        <p className="text-sm opacity-80 whitespace-nowrap">
                            {showSidePanel ? (isID ? "Tekan X untuk tutup panel" : "Press X to close panel") : stepData.desc}
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl"
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="flex items-center gap-4 text-sm">
                                <span className="flex items-center gap-1"><Star className="w-4 h-4 text-amber-500" fill="currentColor" /><strong>{selectedMentor.rating}</strong></span>
                                <span className="text-primary/30">•</span>
                                <span className="flex items-center gap-1"><MapPin className="w-4 h-4 text-primary/50" />{selectedMentor.city}</span>
                                <span className="text-primary/30">•</span>
                                ...
                                <div className="space-y-4">
                                    <div className="bg-amber-50 rounded-2xl p-4 border-2 border-amber-200">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-bold text-lg">{selectedTime} - {parseInt(selectedTime.split(':')[0]) + 1}:00</span>
                                        </div>
                                    </div>
                            ...
                            {bookingStep === 'success' && (
                                <div className="space-y-4 text-center">
                                    ...
                                    <div className="bg-slate-50 rounded-2xl p-4 text-left">
                                        ...
                                        <p className="font-black text-2xl tracking-widest text-primary">SW-{Date.now().toString().slice(-6)}</p>
                                    </div>
                                    <button onClick={() => {
                                        handleClosePanel();
                                        ...
                                        <Video className="w-5 h-5" /> {isID ? "Masuk Ruang Sowan →" : "Enter Sowan Room →"}
                                    </button>
                                </div>
                            )}
                        </div>
            ...
            {showRoomEntry && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl">
                        ...
                        <button onClick={() => { setShowRoomEntry(false); setShowFeedback(true); }}
                            ...
                            <Video className="w-5 h-5" /> {isID ? "Gabung Ruang Sowan" : "Join Sowan Room"}
                        </button>
            ...
            {showFeedback && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl p-8 max-w-lg w-full mx-4 shadow-2xl">
                        ...
                        <textarea className="w-full p-4 border-2 border-primary/20 rounded-2xl text-sm mb-4 resize-none"
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className="relative mb-8" id="search">
                    ...
                    {searchQuery && (
                        <button onClick={() => setSearchQuery('')} className="absolute right-5 top-1/2 -translate-y-1/2 text-primary/40 hover:text-primary">
                            <X className="w-5 h-5" />
                        </button>
                    )}
                </div>
                    ...
                    {([
                        ...
                        <button key={f.key}
                            onClick={() => {
                                ...
                                if (currentStep === 1) setTimeout(() => setCurrentStep(2), 300);
                            }}
                            className={`px-6 py-3 bg-white rounded-full font-bold text-sm border-2 transition-all whitespace-nowrap ${
                                activeFilter === f.key ? 'border-accent bg-accent/10 shadow-lg shadow-accent/20' : 'border-primary/10 hover:border-primary/30'
                            }`}>
                            {f.label}
                            {f.key !== 'all' && <span className="ml-2 text-primary/40">{MENTORS.filter(m => m.city === f.key).length}</span>}
                        </button>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="card">
                    {filteredMentors.length === 0 ? (
                        <div className="col-span-full text-center py-12">
                            ...
                            <p className="text-muted-foreground text-sm mt-2">{isID ? "Coba kata kunci lain atau hapus filter" : "Try different keywords or remove filters"}</p>
                        </div>
                    ) : filteredMentors.map(mentor => (
                        <button key={mentor.id}
                            onClick={() => handleCardClick(mentor)}
                            className={`relative rounded-[32px] overflow-hidden border-4 bg-white shadow-2xl text-left transition-all hover:shadow-xl hover:-translate-y-1 ${
